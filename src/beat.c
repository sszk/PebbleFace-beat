...
time_t calc_unix_seconds(const struct tm * tick_time)
{
	...
	const int years_since_epoch = tick_time->tm_year - 70;
	const int days_since_epoch  = (years_since_epoch * 365 + (years_since_epoch + 1) / 4 + tick_time->tm_yday);
...
void display_time(const struct tm * tick_time)
{
	...
	const time_t unix_seconds = calc_unix_seconds(tick_time) - utc_offset_seconds;
